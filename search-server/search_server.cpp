...
void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings)
{
    ...
    for (const std::string& word : words) {
        ...
        ids_to_words_freqs_[document_id][word] += inv_word_count;
    }
...
{
    static std::map<std::string, double> map_freq;
    if (document_ids_.count(document_id) == 0)  return map_freq;
...
void SearchServer::RemoveDocument(int document_id)
{
    ...
    for (const auto& [word, data] : ids_to_words_freqs_.at(document_id))
    {
        auto& document_freqs = word_to_document_freqs_.at(word);
